...
- **本地图像描述增强（第二期）**：后台用本地 VLM 为图片生成结构化中文描述
  （背景/颜色/画风/内容/人物/物体/图中文字），描述经 Qwen3-Embedding-0.6B
  编码为字段化行向量，与 CLIP 两路做 RRF 融合检索，细粒度查询显著增强；
  caption 向量命中的结果带绿色角标"述"，悬停显示命中的描述行
- **GGUF/llama.cpp 后端（8GB 卡速度甜点）**：caption 默认使用
  Qwen3-VL-4B-Instruct Q4_K_M GGUF + llama.cpp 官方预编译 llama-server
  （项目内自包含，无需编译/装包）。实测 6.3 秒/张（vs transformers fp16
  2B 的 59 秒/张，约 9 倍提速），全量 683 张约 70 分钟；JSON 语法约束 +
  阶梯重试保证成功率
- **交叉编码精排（可选）**：Qwen3-Reranker-0.6B 对融合结果 top-30 做
  yes/no token 打分重排（官方 LogitScore 实现，零额外权重），命中率提升
  约 1~5%，热查询仅多 ~70ms
